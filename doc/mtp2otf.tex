...
\]
It is encoded in cumulative GPOS layers. TFM pair kerns are represented
by the OpenType \file{kern} feature in GPOS. A missing TFM pair contributes
...
\textbf{Word.}
The reviewed setup uses TTF. Select \file{MTPro2 Math} or
\file{MTPro2 Math Lite} as the equation font and enable font kerning
for sizes of 1~pt and above. This is a font-size threshold, not the
amount of adjustment. Word is the primary application target; its
GPOS-based output has been checked against source data and rendered
comparisons. Equation run boundaries can limit pair positioning.

\textbf{LuaLaTeX.}
Use the generated OTF with \file{unicode-math}. The font contains the
source-derived GPOS spacing, but the tested LuaHBTeX math paths do not
fully apply it. These spacing differences arise from engine processing,
not missing font data. For the original TeX spacing and optical-size
selection, we recommend the MTPro2 Type~1 fonts with the \file{mtpro2}
package.

\textbf{Specimens.}
\file{doc/word.docx} and \file{doc/lualatex.tex} are editable sources for
\file{doc/word.pdf} and \file{doc/lualatex.pdf}. Both show Full with
Nimbus donors. Word uses TTF with Times New Roman body text;
LuaLaTeX uses OTF with Nimbus Roman body text.
